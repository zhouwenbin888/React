<template>
  <div class="biaodan">
    ...
    <div style="position:relative;margin-bottom: 80px;">
      <a style="margin: 0px 0px 0px 20px ;color:#c62922;" @click="this.$router.push('/remeber1');">忘记密码</a>
      <a style="margin: 0px 20px 0px 0px ;right: 0;position:absolute;color:#c62922;" @click="this.$router.push('/teacheregist');">注册</a>
    </div>
  </div>
</template>
<style scoped>
      ...
      axios.post('/api/teacher/login', formData)
          .then(response => {
            ...
            if (response.data.msg === "success") {
              ...
              this.$router.push(`/teacherhome/${id}`);
            }
          })
          .catch(error => {
            console.log(error);
