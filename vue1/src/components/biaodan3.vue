<template>
  <div class="biaodan">
    <input class="input1" type="text" placeholder="手机号" v-model="account" @focus="clearError('accountError')"
      ref="input1" />
    ...
    <div style="position:relative;margin-bottom: 80px;">
      <a style="margin: 0px 0px 0px 20px ;color:#c62922;" @click="this.$router.push('/remeber2');">忘记密码</a>
      <a style="margin: 0px 20px 0px 0px ;right: 0;position:absolute;color:#c62922;" @click="this.$router.push('/studentregist');">注册</a>
    </div>
  </div>
</template>
<style scoped>
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      formData.append('stu_account', this.account);
      ...
      switch (true) {
        case (this.account === ""):
          this.addInputError('input1', 'accountError', '*请输入手机号');
          break;
        case (!phoneRegex.test(this.account)):
          this.addInputError('input1', 'accountError', '*请输入正确手机号');
          break;
        case (this.account !== "1146872611@qq.com"):
          this.addInputError('input1', 'accountError', '*账户错误');
          break;
        default:
      ...
      switch (true) {
        case (this.password === ""):
          ...
          break;
        case (this.password !== "111"):
          this.addInputError('input2', 'passwordError', '*手机号或密码错误');
          break;
