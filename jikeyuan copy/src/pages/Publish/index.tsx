...
const Publish = () => {
  ...
  const onfinish = (data: article) => {
    ...
    const reqdata2 = { ...reqdata, id: articleId }
    if (imageList.length !== type) { return message.warning('封面类型和图片不匹配') }
    else if (articleId !== null) {
      axios.put(`http://geek.itheima.net/v1_0/mp/articles/${reqdata2.id}?draft=false`, reqdata2, config)
        .then(res => {
          console.log(res)
        })
      ...
      axios.post('http://geek.itheima.net/v1_0/mp/articles?draft=false', reqdata, config)
        .then(res => {
          console.log(res)
        })
  ...
  useEffect(() => {
    if (articleId !== null) {
      axios.get(`http://geek.itheima.net/v1_0/mp/articles/${articleId}?draft=false`, config)
        .then(res => {
          form.setFieldsValue({
            ...res.data.data,
          ...
          setImageList(res.data.data.cover.images.map((url: string) => {
            return { url }
          }))
        })
    }
