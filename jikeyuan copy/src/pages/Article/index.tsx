...
const Article = () => {
  ...
  const confirm = (data: data2) => {
    ...
    axios.delete(`http://geek.itheima.net/v1_0/mp/articles/${data.id}`, config).then(res =>
      setdata({
        ...reqdata
      })
    )
  ...
  useEffect(() => {
    axios.get('http://geek.itheima.net/v1_0/mp/articles', config).then(res => {
